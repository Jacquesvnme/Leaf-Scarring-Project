...
# image_paths = ['Sample Data/data1.png']
# image_paths = ['Sample Data/4&8Square.jpg']
# image_paths = ['Sample Data/4&8SquareCopy.jpg']
...
for image_path in image_paths:
    ...
    if frame is not None:
        ...
        if len(contours) > 0:
            ...
            # Check if the contour is reasonable
            if w > 0 and h > 0:
                cube_size_in_pixels = max(w, h)  # Taking the larger side
                pixel_to_cm_ratio = cube_size_in_pixels / 5.0  # 2 cm cube
                print(f'Image: {image_path}')
                print(f'Pixel-to-cm Ratio: {pixel_to_cm_ratio} pixels/cm')

                # Calculate the cube area in pixels and cm²
                cube_area_pixels = w * h
                cube_area_cm2 = cube_area_pixels / (pixel_to_cm_ratio ** 5)
                percentage_of_4cm2 = (cube_area_cm2 / 25) * 100  # Working out the accuracy

                print(f'Cube Area: {cube_area_cm2:.2f} cm²')
                print(f"The cube area is {percentage_of_4cm2:.2f}% of a 4 cm² cube.")  # Displaying the accuracy
                ...
                labeled_resized_cube_mask = add_label(cv2.cvtColor(resized_cube_mask, cv2.COLOR_GRAY2BGR),
                                                      'Calibration Cube')
                ...
                row2 = cv2.hconcat(
                    [labeled_resized_unhealthy, labeled_resized_healthy, labeled_resized_cube_mask])  # Second row
